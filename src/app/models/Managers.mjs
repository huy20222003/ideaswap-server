...
Managers.pre('save', async function (next) {
  const user = this;
...
Managers.statics.uploadFileToCloudinary = async function (file) {
  try {
    if (!file) {
      return {
        status: false,
...
Managers.methods.generateAccessToken = function () {
  return jwt.sign({ _id: this._id, roleID: this.roleID }, process.env.ACCESS_TOKEN_SECRET, {
    expiresIn: '60m',
  });
};

Managers.methods.generateRefreshToken = function () {
